...
class UsuarioViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        """
        if self.request.user.is_authenticated and self.request.user.email=='admin@email.com':
    ...
    @action(detail=False, methods=['post'], url_path='refresh', permission_classes=[AllowAny], authentication_classes=[])
...
class PasswordResetViewSet(viewsets.GenericViewSet):
    ...
    def solicitar(self, request):
        ...
        reset_link = f"http://127.0.0.1:5500/reset_senha.html?uid={uidb64}&token={token}"
